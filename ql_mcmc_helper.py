...
class CartPole:
    ...
    def run_single_episode(self,ep_no,w):
        ...
        while True:
            ...
            qprobs=np.exp(probs)
            qprobs=qprobs/np.sum(qprobs)

            action = np.random.choice(self.nA,p=qprobs[0])
            next_state,reward,done,_ = self.env.step(action)
            next_state = next_state[None,:]
            #print(probs)
            action = 1 if np.random.uniform() < qprobs[0][1] else 0 # roll the dice!
            self.aprobs.append(action - probs[0][1])
            self.q_updates.append(reward+self.GAMMA*np.amax(self.neuralnet.ForwardPass(next_state,w)))
        ...
        for i,item in enumerate(self.rewards):
            ...
            self.discountedsum.append(sum([ r * (self.GAMMA ** t) for t,r in enumerate(self.rewards[i:])]))
    ...
    def sumrewards(self):
        return self.score
    def getrewards(self):
        return np.copy(np.array(self.rewards))
    def loss(self):
        return np.copy(self.aprobs)
    def actual(self):
        return np.copy(self.q_updates)
    def predicted(self):
